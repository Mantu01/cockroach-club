  ...
  return (
    <div className="flex flex-1 flex-col">
      ...
      <div className="grid grid-cols-2 gap-3 px-4 py-4 lg:grid-cols-4 lg:px-6">
        <StatCard label="Jobs Searched" value={stats?.jobsSearched ?? 0} trend="up" change="+12%" hint="Total search queries" />
        <StatCard label="Applications" value={stats?.totalApplications ?? 0} trend="up" change="+8%" hint="Tracked applications" />
        <StatCard label="Prep Done" value={stats?.preparationsDone ?? 0} trend="stable" change={`${stats?.prepProgress ?? 0}%`} hint="Questions completed" />
        <StatCard label="Jobs Scraped" value={stats?.totalJobsScraped ?? 0} trend="up" change="Live" hint="Available in database" />
      </div>
          ...
          <CardContent className="px-2 pb-4">
            <ChartContainer config={chartConfig} className="aspect-auto h-[180px] w-full">
              <AreaChart data={chartData}>
                ...
                <Area dataKey="searches" type="natural" fill="var(--color-searches)" fillOpacity={0.2} stroke="var(--color-searches)" stackId="a" />
                <Area dataKey="applications" type="natural" fill="var(--color-applications)" fillOpacity={0.2} stroke="var(--color-applications)" stackId="a" />
              </AreaChart>
            </ChartContainer>
          </CardContent>
        </Card>
        ...
        <Card className="border border-border/40 bg-muted/5">
          ...
          <CardContent className="flex flex-col gap-3 px-4 pb-4">
            {predictions.map((pred) => {
              ...
              return (
                <div key={pred.label} className="flex items-center justify-between gap-2 border-b border-border/20 pb-2 last:border-0 last:pb-0">
                  <span className="text-xs text-muted-foreground">{pred.label}</span>
                  <div className="flex items-center gap-1.5">
                    <span className="text-sm font-black tabular-nums" style={{ fontFamily: "'Syne', sans-serif" }}>
                      {pred.value}%
                    </span>
      ...
      <div className="px-4 py-4 lg:px-6">
        <Card className="border border-border/40 bg-muted/5">
          <CardHeader className="flex flex-row items-center justify-between px-4 py-3">
            ...
            <Badge variant="outline" className={UI_SIZES.badge}>{activity.length} items</Badge>
          </CardHeader>
              ...
              <TableBody>
                {activity.map((item) => (
                  <TableRow key={item._id}>
                    ...
                    <TableCell className="text-xs text-muted-foreground hidden sm:table-cell">{item.searchQuery}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
