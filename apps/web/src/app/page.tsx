        ...
        <>
          <div className="flex flex-col justify-center px-10 py-12 lg:px-16 border-r border-border/30">
            <div className="flex items-center gap-3 mb-5">
              <span className="font-mono text-xs tracking-[0.3em] text-muted-foreground">{stage.id}</span>
              <div className="h-px flex-1 bg-border/40" />
              <Badge variant="outline" className="text-[10px] tracking-widest uppercase font-medium px-2 py-0.5">
                {stage.sub}
              </Badge>
            </div>
            <h2
            ...
            <div className="flex items-center gap-2 mt-6">
              ...
              <span className="text-xs font-medium tracking-wider uppercase" style={{ color: stage.accent }}>
                Phase {stage.id}
              </span>
            </div>
          </div>
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    link.href = "https://fonts.googleapis.com/css2?family=Syne:wght@400;700;800;900&family=DM+Serif+Display:ital@0;1&family=DM+Mono:wght@300;400&display=swap";
    link.rel = "stylesheet";
    document.head.appendChild(link);
  }, []);

  return (
    <main className="w-full overflow-x-hidden" style={{ fontFamily: "'DM Serif Display', serif" }}>
      <section className="w-full min-h-[92vh] flex flex-col justify-between border-b border-border/30 px-6 lg:px-16 pt-16 pb-10">
        ...
        >
          ...
          >
            ...
            <span className="block italic" style={{ fontFamily: "'DM Serif Display', serif", fontWeight: 400 }}>
              that refused
            </span>
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mt-12">
            ...
            >
              Built for those the system already counted out. We help you write the resume no bot dares ignore,
              prep the answers no interviewer expects, and crawl into opportunities that were never meant for you.
            </p>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-0 divide-x divide-border/30">
          {stats.map((stat, i) => (
            <div key={i} className="px-6 lg:px-10 py-4 flex flex-col gap-1">
              ...
              <span className="text-xs text-muted-foreground leading-snug max-w-[160px]">{stat.label}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="w-full border-t border-border/30 px-6 lg:px-16 py-20 flex flex-col lg:flex-row items-center justify-between gap-10">
        <div className="flex flex-col gap-4 max-w-xl">
          ...
          >
            "Cockroaches don't survive because they're the strongest. They survive because they adapt faster
            than anything designed to kill them."
          </blockquote>
          <span className="text-xs text-muted-foreground font-mono">— The Cockroach Mode Doctrine</span>
        </div>
        <Button
          size="lg"
